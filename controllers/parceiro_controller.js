...
function create(parceiro) {
  const params = [parceiro.getNome(), parceiro.getWebsite(), parceiro.getContato()];
  let sql = 'INSERT INTO parceiros (nome, website, contato) VALUES (?, ?, ?)';
  db.query(sql, params, function (err, result) {
    if (err) throw err;
    console.log('Parceiro criado com sucesso. ID do parceiro: ' + result.insertId);
  });
}
...
async function retrieve(id) {
  ...
  return new Promise((resolve, reject) => {
    db.query(sql, async function (err, result) {
      if (err) throw err;
      const item = result[0];
      const parceiro = new Parceiro(item.id, item.nome, item.website, item.contato);
      resolve(parceiro);
    });
  });
}
...
function update(parceiro) {
  ...
  var sql = 'UPDATE parceiros SET nome = ?, website = ?, contato = ? WHERE id = ?';
  db.query(sql, [nome, website, contato, id], function (err, result) {
    if (err) throw err;
    console.log('Parceiro atualizado com sucesso. Registros atualizados: ' + result.affectedRows);
  });
}
...
function destroy(id) {
  ...
  db.query(sql, [id], function (err, result) {
    if (err) throw err;
    console.log('Parceiro excluído com sucesso. Registros excluídos: ' + result.affectedRows);
  });
}
  ...
  return new Promise((resolve, reject) => {
    db.query('SELECT * FROM parceiros', async function (error, collection) {
      for (const item of collection) {
        let parceiro = new Parceiro(item.id, item.nome, item.website, item.contato);
